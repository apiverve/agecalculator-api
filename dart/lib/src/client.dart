...
class AgecalculatorValidationException implements Exception {
  final List<String> errors;

  AgecalculatorValidationException(this.errors);

  @override
  String toString() => 'AgecalculatorValidationException: ${errors.join("; ")}';
}
...
/// Age Calculator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/agecalculator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [dob] (required) - The date of birth to calculate the age from (format: YYYY-MM-DD) [format: date]
class AgecalculatorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'dob': ValidationRule(type: 'string', required: true, format: 'date'),
  };

  AgecalculatorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/agecalculator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AgecalculatorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AgecalculatorValidationException(errors);
    }
  }

  /// Execute a request to the Age Calculator API.
  ///
  /// Parameters are validated before sending the request.
  Future<AgecalculatorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AgecalculatorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AgecalculatorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AgecalculatorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AgecalculatorException('Resource not found');
      } else {
        throw AgecalculatorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AgecalculatorException) rethrow;
      throw AgecalculatorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Age Calculator API client.
class AgecalculatorException implements Exception {
  final String message;

  AgecalculatorException(this.message);

  @override
  String toString() => 'AgecalculatorException: $message';
}
